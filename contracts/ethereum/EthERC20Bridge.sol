...
import "./interfaces/IEthERC20Bridge.sol";
...
contract EthERC20Bridge is IEthERC20Bridge, TokenAddressHandler {
    ...
    function transferExitAndCall(
        ...
        address expectedSender = redirectedAddress == address(0) ? initialDestination : redirectedAddress;
        ...
        if (data.length > 0) {
            ...
            bytes4 res = IExitTransferCallReceiver(to).onExitTransfered(expectedSender, amount, erc20, data);
            require(res == IExitTransferCallReceiver.onExitTransfered.selector, "EXTERNAL_CALL_FAIL");
        }
     ...
     */
    function callStatic(address targetContract, bytes4 targetFunction) internal view returns (bytes memory) {
    ...
    function deposit(
        ...
        {
            bool isDeployed;
            (isDeployed, depositCalldata) = getDepositCalldata(erc20, msg.sender, destination, amount, callHookData);
