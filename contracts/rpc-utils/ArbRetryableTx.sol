...
interface ArbRetryableTx {
    ...
    * If this reverts, txId is still available for redemption (until it times out or is canceled).
     ...
     */
    function getTimeout(bytes32 txId) external view returns (uint256);
     ...
     * @return (price, nextUpdateTimestamp). Price is guaranteed not to change until nextUpdateTimestamp.
     */
    function getKeepalivePrice(bytes32 txId) external view returns (uint256, uint256);
    ...
    */
    function keepalive(bytes32 txId) external payable returns (uint256);
    ...
    */
    ...
    event LifetimeExtended(bytes32 indexed txId, uint256 newTimeout);
